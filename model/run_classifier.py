...
class DataProcessor(object):
    ...
    def _read_tsv(cls, input_file, quotechar=None):
        ...
        with open(input_file, "r", encoding="utf-8") as f:
            ...
            lines = []
            for line in reader:
                if sys.version_info[0] == 2: # python vesion = 2
...
class QnliProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            text_a = line[1]
            text_b = line[2]
            if line[-1] in ["entailment", "not_entailment"]:
                label = line[-1]
            else:
                label = "entailment"
            examples.append(
...
def main():
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if output_mode == "classification":
            ...
            all_label_ids = torch.tensor([f.label_id for f in tes_features], dtype=torch.float)
        ...
        output_test_file = os.path.join(args.output_dir, "QNLI.tsv")
        with open(output_test_file, "w") as writer:
            logger.info("***** Test results *****")
            writer.write("index"+'\t'+"prediction"+'\n')
            for i in range(len(preds)):
                writer.write(str(i)+'\t'+["entailment", "not_entailment"][preds[i]]+'\n')
